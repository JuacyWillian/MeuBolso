...
def register(name, prefix, ttf_fname, fontd_fname):
    """Register an Iconfont
    :param name: font nome identifier.
    :param prefix: prefix of icons strings. ex: 'fa-' for font-awesome.
    ...
    with open(fontd_fname, 'r') as f:
        ...
        _register[name] = ttf_fname, fontd_fname, fontd, prefix


def icon(code, size=None, color=None, font_name=None):
    """ Gets an icon from iconfont.
    :param code: Icon codename (ex: 'icon-nome')
    :param size: Icon size
    :param color: Icon color
    :param font_name: Registered font nome. If None first one is used.
    ...
    if size is not None: s = "[size=%s]%s[/size]" % (size, s)
    if color is not None: s = "[color=%s]%s[/color]" % (color, s)
    return s
...
def _parse(data):
    ...
    # iterate rule indices and extract valor
